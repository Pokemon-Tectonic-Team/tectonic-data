...
const PUBLIC_VERSION_COMMIT = "15ca404c46f0fb00a76eac40fb8865d9b0320a27";

const REPO_ROOT = path.join(__dirname, "..");
const DIST_DIR = path.join(REPO_ROOT, "dist");
const MAPPINGS_PATH = path.join(REPO_ROOT, "pokePartyMappedEncodings.json");
...
async function loadData(dev: boolean = false): Promise<void> {
    ...
    const outputName = dev ? "loadedData-dev.json" : "loadedData.json";
    await writeFile(path.join(DIST_DIR, outputName), JSON.stringify(loadedData));
    ...
    // Read, update, and write back the PokeParty mappings
    const mappings = JSON.parse(await readFile(MAPPINGS_PATH, "utf-8"));
    function addNewMappings(tectonicKeys: string[], mappingCategory: string) {
        ...
        let nextMappingId = Object.keys(mapping).length + 1; // Note - mapping ids start at 1, 0 indicates undefined/null
    ...
    await writeFile(MAPPINGS_PATH, mappingsJson);
    await writeFile(path.join(DIST_DIR, "pokePartyMappedEncodings.json"), mappingsJson);
}
